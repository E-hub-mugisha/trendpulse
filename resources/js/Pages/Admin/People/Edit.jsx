// resources/js/Pages/Admin/People/Edit.jsx
...
export default function Edit({ story: storyData, categories }) {
    const fileInputRef = useRef(null);
    const [preview, setPreview] = useState(storyData.featured_image);

    const { data, setData, post, processing, errors } = useForm({
        title: storyData.title || '',
        person_name: storyData.person_name || '',
        relationship_status: storyData.relationship_status || '',
        excerpt: storyData.excerpt || '',
        story: storyData.story || '',
        featured_image: null,
        category_id: storyData.category_id || '',
        is_featured: storyData.is_featured,
        is_popular: storyData.is_popular,
        is_published: storyData.is_published,
        published_at: storyData.published_at ? storyData.published_at.slice(0, 10) : '',
        _method: 'put',
    });

    const submit = (e) => {
        e.preventDefault();
        post(`/admin/people/${storyData.slug}`, { forceFormData: true });
    };

    const handleImage = (e) => {
        const file = e.target.files[0] ?? null;
        setData('featured_image', file);
        setPreview(file ? URL.createObjectURL(file) : storyData.featured_image);
    };
                ...
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="mt-1 text-sm text-gray-500">
                            {storyData.views.toLocaleString()} views
                        </p>
                    </div>

                    
                    <a    href={`/people/${storyData.slug}`}
                        target="_blank"
                ...
                <form onSubmit={submit} className="mt-8 space-y-6 rounded-2xl border border-gray-100 bg-white p-6 sm:p-8">
                    ...
                    <div>
                        ...
                        <div
                            ...
                            className="mt-2 flex aspect-[4/3] max-w-xs cursor-pointer items-center justify-center overflow-hidden rounded-2xl border-2 border-dashed border-gray-200 bg-gray-50 hover:border-gray-300"
                        >
                            {preview ? (
                    ...
                    <div className="grid gap-6 sm:grid-cols-2">
                        ...
                        <div>
                            ...
                            <input
                                ...
                                className="mt-2 block w-full rounded-xl border-0 bg-gray-100 px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-black"
                            />
                            {errors.person_name && <p className="mt-1 text-xs text-red-500">{errors.person_name}</p>}
                        </div>
                        ...
                        <div>
                            ...
                            <input
                                ...
                                className="mt-2 block w-full rounded-xl border-0 bg-gray-100 px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-black"
                            />
                            {errors.relationship_status && <p className="mt-1 text-xs text-red-500">{errors.relationship_status}</p>}
                        </div>
